...
//        let happenOffsetY = self.happenOffsetY
        ...
        let pullingPercent = (currentOffsetY - happenOffsetY) / h
        ...
        if scrollView.dragging {
            self.pullingPercent = pullingPercent
            // 普通 和 即将刷新 的临界点
            let normal2pullingOffsetY = happenOffsetY + h
    ...
    override func scrollViewContentSizeDidChange(change: NSDictionary?) {
        ...
        // 内容的高度
        let contentHeight = scrollView.contentH + ignoredScrollViewContentInsetBottom
        // 表格的高度
        let scrollHeight = scrollView.h - scrollViewOriginalInset.top - scrollViewOriginalInset.bottom + ignoredScrollViewContentInsetBottom
        // 设置位置和尺寸
        ...
        set {
            ...
            if newValue == .NoMoreData || newValue == .Idle {
                ...
                UIView.animateWithDuration(RefreshFastAnimationDuration,
                    animations: {
                        var bottom = self.h + self.scrollViewOriginalInset.bottom
                        ...
                        self.scrollView.offsetY = self.happenOffsetY + self.h
